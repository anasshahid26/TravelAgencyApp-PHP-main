...
// get Room images
  ...
  //get image my function

    function myGetimg($geturl){

        $id = substr($geturl, strrpos($geturl, '/') + 1);

        $query = $this->db->select('pt_hotels');
        $this->db->where('hotel_slug', $id);
...
// Get Room Type to show instead of title
  ...
  public function suggestionResults($query) {
    ...
    $hotels = array();
    $locations = array();
    $this->db->select('id,location,country');
    ...
    if (!empty($locres)) {
      ...
      foreach ($locres as $l) {
        ...
        $locations[] = (object) array('id' => $l->id, 'country' =>$l->country, 'name' => trim($locInfo->city)." , ".$l->country, 'module' => 'location', 'disabled' => false);
      }
    }
    if (!empty($res)) {
...
//$hotels[] =  (object)array('id' => array('dataType' => 'hotels', 'id' => $r->id), 'name' => trim($title), 'module' => 'hotels');
    ...
    $response = array_merge($locations,$hotels );
